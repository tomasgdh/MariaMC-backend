...
namespace WebApiMariaMC.Controllers
{
    ...
    public class IngresoController : ControllerBase
    {
        ...
        public IngresoController(Maria_MCContext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<object>> GetAll(int idSucursal)
        {
            try
            {
                var lista = await _context.Ingreso
                    .Include(vfp => vfp.IdTipoDeIngresoNavigation)
                    ...
                    .Where(c => c.IdSucursal == idSucursal)
                                   .Select(td => 
                                    new { id = td.Id,
                                        ...
                                        idTipoMovimiento = td.IdTipoDeIngreso,
                                        tipoMovimientoDesc = td.IdTipoDeIngresoNavigation.Descripcion,
        ...
        public async Task<ActionResult<object>> GetAllPaginado(int idSucursal, int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                var totalRecords = await _context.Ingreso.Where(c => c.IdSucursal == idSucursal).CountAsync();
                var lista = await _context.Ingreso
                    .Include(vfp => vfp.IdTipoDeIngresoNavigation)
                    .Include(vfp => vfp.IdTipoFormaPagoNavigation)
                    .Where(c => c.IdSucursal == idSucursal)
        ...
        [NonAction]
        [HttpGet(Name = "GetIngreso")]
        public async Task<ActionResult<object>> Get(int id)
        {
            var tipo = await _context.Ingreso.FindAsync(id);
            ...
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var nuevoItem = new
                    {
                        ...
                        item.Descripcion,
                        item.IdTipoDeIngreso,
                        tipoMovimientoDesc = item.IdTipoDeIngresoNavigation.Descripcion,
        ...
        public async Task<ActionResult<object>> Update(ItemMovimientoDeCajaRequest itemDTO)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    Ingreso? tdAModificar = await _context.Ingreso.FindAsync(itemDTO.id);
                    ...
                    CuentaCorriente? cc = await _context.CuentaCorrientes
                        ...
                        && i.IdTipoDeMovimiento == "G" /*Ingreso*/
                    ...
                    tdAModificar.IdTipoDeIngreso = itemDTO.idTipoMovimiento;
