...
namespace WebApiMariaMC.Controllers
{
    ...
    public class TipoIngresoController : ControllerBase
    {
        ...
        public TipoIngresoController(Maria_MCContext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<object>> GetAllActivos()
        {
            try
            {
                var lista = await _context.TipoIngreso
                ...
                var selectItem = new { id = 0, description = "Seleccione un Tipo de Ingreso" };
        ...
        [NonAction]
        [HttpGet(Name = "GetTipoIngreso")]
        public async Task<ActionResult<object>> Get(int id)
        {
            var tipo = await _context.TipoIngreso.FindAsync(id);
        ...
        public async Task<ActionResult<object>> Add(ItemTablabasicaRequest itemDTO)
        {
            try
            {
                TipoIngreso item = new TipoIngreso
                {
        ...
        public async Task<ActionResult<object>> Update(ItemTablabasicaRequest itemDTO)
        {
            try
            {
                TipoIngreso? tdAModificar = await _context.TipoIngreso.FindAsync(itemDTO.id);
